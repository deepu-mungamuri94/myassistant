...
const Credentials = {
        ...
        list.innerHTML = Object.keys(groupedByTag).sort().map(tag => {
            ...
            return `
                ...
                <details class="credential-tag-group bg-white rounded-lg border border-gray-200 overflow-hidden" open>
                    <!-- Tag Header -->
                    <summary class="cursor-pointer px-4 py-2.5 bg-gradient-to-r from-blue-200 to-cyan-200 hover:from-blue-300 hover:to-cyan-300 transition-colors flex justify-between items-center" onclick="Credentials.toggleTag('${Utils.escapeJsAttr(tag)}')">
                                ...
                                <div class="px-4 py-3 hover:bg-blue-50 transition-colors">
                                    ...
                                    <div class="flex justify-between items-center mb-1">
                                        ...
                                        <div class="flex gap-1 ml-2 flex-shrink-0">
                                            <button onclick="Credentials.viewCredentialSecure(${cred.id});" class="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg transition-all" title="View">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                                </svg>
                                            </button>
                                            <button onclick="Credentials.editCredentialSecure(${cred.id});" class="p-1.5 text-green-600 hover:bg-green-100 rounded-lg transition-all" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                </svg>
                                            </button>
                                            <button onclick="Credentials.deleteCredentialSecure(${cred.id})" class="p-1.5 text-red-500 hover:bg-red-100 rounded-lg transition-all" title="Delete">
    ...
    /**
     * View credential with authentication check
     */
    async viewCredentialSecure(id) {
        const authenticated = await window.Security.requireAuthentication('View Credential', 'credentials');
        if (authenticated) {
            window.viewCredential(id);
        }
    },
    
    /**
     * Edit credential with authentication check
     */
    async editCredentialSecure(id) {
        const authenticated = await window.Security.requireAuthentication('Edit Credential', 'credentials');
        if (authenticated) {
    ...
    /**
     * Delete credential with authentication check
     */
    async deleteCredentialSecure(id) {
        const authenticated = await window.Security.requireAuthentication('Delete Credential', 'credentials');
        if (authenticated) {
